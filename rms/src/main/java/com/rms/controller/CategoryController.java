...
public class CategoryController {
	...
	public ResponseEntity<HttpResponse> getAllCategory(){
		try {
			return new ResponseEntity<>(new HttpResponse(HttpStatus.OK.value(),DATA_SUCCESS,categoryService.getAllCategory()), HttpStatus.OK);
	...
	public ResponseEntity<HttpResponse> getCategoryById(@PathVariable Long id){
		try {
			return new ResponseEntity<>(new HttpResponse(HttpStatus.OK.value(),DATA_SUCCESS,categoryService.getCategoryById(id)),HttpStatus.OK);
		}catch(BusinessLogicException e) {
			return new ResponseEntity<>(new HttpResponse(HttpStatus.BAD_REQUEST.value(), e.getMessage()), HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<HttpResponse> deleteCategory(@PathVariable Long id){
		try {
			return new ResponseEntity<>(new HttpResponse(HttpStatus.OK.value(),categoryService.deleteCategory(id)),HttpStatus.OK);
		}catch(BusinessLogicException e) {
			return new ResponseEntity<>(new HttpResponse(HttpStatus.BAD_REQUEST.value(), e.getMessage()), HttpStatus.BAD_REQUEST);
		}
	}
